...
export default function Card({ details, category }: CardProps) {
  const { imageUrl = '/placeholder.png', price = 0, discountedPrice = 0, description = '', inStock = 0, name = '', productId = '', size='' } = details;
    ...
    <div className='flex justify-center'>
      <Link href={`/products/${category}/${productId}`} className="c-card m-4 block bg-white text-black shadow-lg hover:shadow-2xl rounded-lg transition-transform transform hover:scale-105 p-4 h-96 w-64 overflow-scroll">
        ...
        <div className='flex justify-center items-center p-2'>
          <ChakraImage src={imageUrl} alt={name} boxSize={40} fit={'contain'} borderRadius="md" />
        </div>
        <div className="p-4">
